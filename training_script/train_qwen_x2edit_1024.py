...
from src.custom_dataset_qwen_1024 import DataModuleCustom 
...
def main(args):
    ...
    if args.pretrained_lora_path is not None:
        ...
        task_embeddings_lora_path = args.pretrained_lora_path.replace("lora.safetensors","task_embeddings.bin")
        ...
        for name, attn_processor in transformer.attn_processors.items():
            ...
            if name.startswith("transformer_blocks") and layer_index in double_blocks_idx:
                ...
                lora_attn_procs[name] = MultiDoubleStreamBlockLoraProcessor(
                    dim=3072, rank=args.rank, network_alpha=args.network_alpha, device=accelerator.device, dtype=weight_dtype, num_experts=args.experts
                )
                ...
                lora_attn_procs[name].to(accelerator.device, dtype=weight_dtype) 
    ...
    transformer_proj = disp_loss(transformer,task_embeddings_lora,use_Dispersive_Loss=False)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer_proj):
                ...
                height_ = int(pixel_values.shape[-2])
                width_ = int(pixel_values.shape[-1])
                img_shapes_new = [[(1, height_ // vae_scale_factor//2, width_ // vae_scale_factor//2 ),\
                    (1, height_ // vae_scale_factor//2, width_ // vae_scale_factor//2 )]] * bsz # [(2, 32, 32)]
                ...
                model_pred = QwenImagePipeline._unpack_latents(
                    model_pred, height_, width_, vae_scale_factor
                )
