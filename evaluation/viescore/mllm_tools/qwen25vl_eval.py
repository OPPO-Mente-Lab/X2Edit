...
def convert_file_path_to_base64(file_path):
    with open(file_path, "rb") as image_file:
        # 读取文件内容
        image_data = image_file.read()
        
        # 将二进制数据编码为 base64 字节串
        base64_bytes = base64.b64encode(image_data)
        
        # 将字节串转换为 UTF-8 字符串
        base64_encoded_data = base64_bytes.decode("utf-8")
...
class Qwen25VL():
    def __init__(self,model,processor,device="cuda") -> None:     
    ...
    def prepare_prompt(self, image_links: List = [], text_prompt: str = ""):
        ...
        for img_link in image_links:
            if type(img_link) == str:
                image_links_base64.append(convert_file_path_to_base64(img_link))
            else:
