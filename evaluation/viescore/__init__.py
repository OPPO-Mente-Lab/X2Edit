...
import math
import viescore.vie_prompts

from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor

class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None,device="cuda") -> None:
        self.task = task
        ...
        if self.backbone_name == "gpt4o":
            from viescore.mllm_tools.openai1 import GPT4o
            self.model = GPT4o(key_path, model_name="gpt-4o")
        elif self.backbone_name == "gpt4v":
            from viescore.mllm_tools.openai import GPT4v
            ...
            model = Qwen2_5_VLForConditionalGeneration.from_pretrained(path,torch_dtype="auto",device_map="auto")
            processor = AutoProcessor.from_pretrained(path)
            self.model = Qwen25VL(model=model,processor=processor,device=device)
        else:
    ...
    def evaluate_image(self, image_prompts, text_prompt, extract_overall_score_only=False, extract_all_score=True, echo_output=False):
