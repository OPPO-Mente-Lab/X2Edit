...
class FluxTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        task_embeddings = attention_outputs[-1]
        ...
        return encoder_hidden_states, hidden_states, cond_hidden_states if use_cond else None,task_embeddings
...
class FluxTransformer2DModel(
    ...
    def forward(
        ...
        # print(f"*********** {txt_ids.shape = }, {img_ids.shape = }, {ids.shape = }")
        if is_hidden_add:
            task_ids = torch.zeros((1,3)).to(ids.device,ids.dtype)
            ids = torch.cat((ids, task_ids), dim=0)
            hidden_states_ids = torch.zeros_like(hidden_states[:,0].unsqueeze(0)).to(ids.device,ids.dtype)
            hidden_states = torch.cat((hidden_states, hidden_states_ids), dim=1)
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                encoder_hidden_states, hidden_states, cond_hidden_states ,task_embeddings = torch.utils.checkpoint.checkpoint(
                ...
                encoder_hidden_states, hidden_states, cond_hidden_states,task_embeddings = block(
                    hidden_states=hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cond_hidden_states=cond_hidden_states if use_condition else None,
                    task_embeddings=task_embeddings if use_condition else None,
                    temb=temb,
            ...
            # hidden_states_all.append(torch.cat([hidden_states,cond_hidden_states],dim=-2))
            # controlnet residual
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states, cond_hidden_states = torch.utils.checkpoint.checkpoint(
                    ...
                    temb,
                    cond_temb if use_condition else None,
                    task_embeddings if use_condition else None,
                    image_rotary_emb,
                    **ckpt_kwargs,
                )
        ...
        if is_hidden_add:
            hidden_states = hidden_states[:, encoder_hidden_states.shape[1] :-1, ...]
        else:
        ...
        if not return_dict:
            return output,hidden_states_all
