...
def load_qwenvl(path: str, device: str | torch.device = "cuda", max_length: int = 512):
    # max length 64, 128, 256 and 512 should work (if your sequence is short enough)
    text_encoder = Qwen2_5_VLForConditionalGeneration.from_pretrained(path, torch_dtype=torch.bfloat16).to(device)
    tokenizer = AutoProcessor.from_pretrained(path, padding_side="left", max_length=512, use_fast=True)
    return tokenizer, text_encoder

def load_proj(proj_t5_save_path, device):
    ...
    task_embeddings_lora = nn.Embedding(16, 3072).to(device=device, dtype=torch.bfloat16)
    ...
    return proj_t5.to(device=device, dtype=torch.bfloat16),task_embeddings_lora.to(device=device, dtype=torch.bfloat16)
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def load_qwen_and_proj(self, qwenvl_path, proj_t5_save_path,device):
        self.tokenizer_qwenvl, self.text_encoder_qwenvl = load_qwenvl(qwenvl_path, device)
        self.proj, self.task_embeddings_lora = load_proj(proj_t5_save_path, device)
    ...
    def encode_prompt2(
        ...
        pooled_prompt_embeds, prompt_embeds = self._get_qwenvl_prompt_embeds(
            prompt=prompt,
            image=image,
            num_images_per_prompt=num_images_per_prompt,
            max_sequence_length=max_sequence_length,
    ...
    def prepare_latents(
        ...
        height = height_cond = 2 * (int(height) // self.vae_scale_factor)  
        width = width_cond = 2 * (int(width) // self.vae_scale_factor)
        ...
        shape_subject = (batch_size, num_channels_latents, height_cond, width_cond)  
        ...
        subject_latents = self._pack_latents(subject_image_latents, batch_size, num_channels_latents, height_cond, width_cond)
        mask2 = torch.zeros(shape_subject, device=device, dtype=dtype)
        mask2 = self._pack_latents(mask2, batch_size, num_channels_latents, height_cond, width_cond)
        latent_subject_ids = prepare_latent_subject_ids(height_cond, width_cond, device, dtype)
        latent_subject_ids[:, 1] += offset  # fixed offset
        # latent_subject_ids[:, 1] += min(height_cond,width_cond)  # fixed offset
        subject_latent_image_ids = torch.concat([latent_subject_ids], dim=-2)
    ...
    def __call__(
        ...
        for subject_image in subject_images:
            ...
            subject_image = self.image_processor.preprocess(subject_image)
        ...
        #             return_dict=False,
        #         )[0]
        task_type=torch.tensor(task_id).to(device=latents.device,dtype=torch.int) ### 推理时相应修改
        task_embeddings = self.task_embeddings_lora.to(device=latents.device)(task_type)  #       
        # 6. Denoising loop
