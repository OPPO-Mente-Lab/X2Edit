...
def add_ml_decoder_head(model, num_classes=-1, num_of_groups=-1, decoder_embedding=768, zsl=0):
    ...
    try:
        ...
        num_features = decoder_embedding
    ...
    def __call__(self, h: torch.Tensor, duplicate_pooling: torch.Tensor, out_extrap: torch.Tensor):
        for i in range(h.shape[1]):
            ...
            if len(duplicate_pooling.shape)==3:
                ...
                # w_i = w_i / torch.norm(w_i)
                w_i = w_i / torch.norm(w_i, dim=0, keepdim=True)
            else:
...
class MLDecoder(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
        ...
        if not zsl:
            # query_embed = nn.Embedding(embed_len_decoder, decoder_embedding, _weight=torch.zeros(embed_len_decoder,decoder_embedding))
