...
base_prompts = f'''参考图片的内容，给出合理的文字修改编辑指令的prompts是什么？
        注意指令意图一定是编辑图片作画的意图，要基于参考图片的内容事实，不需要输出中间过程和reasoning。\
        只做中文的添加或者改写或者去除，比如"将标题从'早上好'改为'晚上好'"。\
        这是图片中需要修改的文字:"<text>"。修改了之后的文字，内容和逻辑要合理，修改前后的文字长度不能相差太多。例如"将'孜然肉夹馍'中的'孜然'改为'陕西'"。\
        只需要生成一条编辑指令。
        '''

base_prompts = f'''文字编辑修改：待修改的文字:"<text>"。 修改了之后的文字，内容和逻辑要合理，修改前后的文字长度接近。
        修改前后文字含义或者文字内容可以很大区别。例如"将'孜然肉夹馍'改为'四川麻辣烫'"。
        只需要生成一条编辑指令。
        '''

prompts_ocr = "直接输出图片的ocr内容，不要输出中间过程"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_root', default="/mnt/data/group/text2img_data/data_font/wukong/*", type=str) # /mnt/data/group/text2img_data/data_process/coyo1/{00000..51975}.tar
    ...
    output_dir = f"/mnt/data/group/**/textflux-main/textflux-qwen-generate-ch/{process_id+16}"
        ...
        if is_tar_openable(tar_name.item()):
            ...
            for obj in dl:
                ...
                try:
                    ...
                    if abs(len(text)-len(new_text))>3:
                        continue
                    ...
                    qwen_ins_ocr = qwenvl(output_image_open,model_7b,processor_7b,prompts_ocr,devices)[0]
                    if new_text not in qwen_ins_ocr: 
                        continue
