...
from utils.src_inference.pipeline import FluxPipeline
from utils.src_inference.lora_helper import set_single_lora
...
if __name__ == "__main__":
    ...
    # Initialize model
    model_path = "/mnt/data/group/models/flux/FLUX.1-dev"
    pipe = FluxPipeline.from_pretrained(model_path, torch_dtype=torch.bfloat16).to(devices)

    # Load OmniConsistency model
    set_single_lora(pipe.transformer, 
                    "/mnt/data/group/models/OmniConsistency/OmniConsistency.safetensors", 
                    lora_weights=[1], cond_size=width)
    ...
    tar_lora_dict = {}
    lora_paths = glob.glob('/mnt/data/group/models/OmniConsistency/LoRAs/*.safetensors')
    lora_paths_new = [i for i in lora_paths if i.split('/')[-1].split('_rank')[0].replace('_', ' ') in ['Van Gogh', 'Pixel', 'Chinese Ink', 'Oil Painting','LEGO','Fabric']]
    for i, tar in enumerate(tar_list):
        tar_lora_dict[tar] = lora_paths_new[i % len(lora_paths_new)]

    curr_tar_lists = np.array_split(tar_list, num_processors)[process_id]
    print(len(tar_list))

    output_dir = f"/mnt/data/group/text2img_data/X2I/tars/style_1024/{process_id}" 
    ...
    for tar_name in curr_tar_lists:
        lora_path = tar_lora_dict[str(tar_name)]
        lora_dir = os.path.dirname(lora_path)
        weight_name = lora_path.split('/')[-1]
        qwen_ins_type = weight_name.split('_rank')[0].replace('_', ' ')
        qwen_ins = f'Convert the image to a {qwen_ins_type} style'
        print(f"style type: {qwen_ins_type}")
        ...
        if is_tar_openable(tar_name.item()):
            ...
            for obj in dl:
                ...
                jpg = obj["jpg"][0]
                try:    
                    input_image_open = resize_crop(jpg,width)
                    ...
                    if q_score_1<3.5 or aesthetic_lama_output_1<4.5 or q_score_clip_1<0.6:continue
                    ...
                    output_image_open = pipe(
                                    ...
                                    spatial_images=[input_image_open],
                                    subject_images=[],
                                    cond_size=width,
                                ).images[0]
                    print("## OmniConsistency Time ##:",time.time()-t1)
                    ...
                    # if qwen_ins_type in 
                    # if sum(scores) <= 6: continue
                    
                    xkey = "%09d" % index
                    jsons["instruction"] = qwen_ins
                    jsons["label"] = "OmniConsistency"
                    jsons["task"] = "style change"
